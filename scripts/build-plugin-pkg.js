...
try {
	if (fs.existsSync(lastParam)) {
		pluginDir = fs.realpathSync(lastParam);
		fs.accessSync(pluginDir + "/plugin.json", fs.constants.R_OK);
	} else if (fs.existsSync(`src/plugins/${lastParam}`)) {
		pluginDir = fs.realpathSync(`src/plugins/${lastParam}`);
	} else {
		...
		if (fs.existsSync(distDir)) {
			...
			for (const file of fs.readdirSync(distDir + "/css")) {
				if (file.indexOf(pluginManifest.id + ".") === 0) {
					...
					pluginManifest.dwcFiles.push("css/" + file);
					filesAdded = true;
				}
			}
			for (const file of fs.readdirSync(distDir + "/js")) {
				if (file.indexOf(pluginManifest.id + ".") === 0) {
					...
					pluginManifest.dwcFiles ??= [];
					pluginManifest.dwcFiles.push("js/" + file);
					filesAdded = true;
				}
			}
			...
			const dsfPluginDir = `${pluginDir}/dsf/`;
			if (fs.existsSync(dsfPluginDir)) {
				archive.directory(dsfPluginDir, "dsf");
				filesAdded = true;
			...
			if (fs.existsSync(extraDwcPluginDir)) {
				archive.directory(extraDwcPluginDir, "dwc");
				filesAdded = true;
			...
			if (fs.existsSync(sdPluginDir)) {
				archive.directory(sdPluginDir, "sd");
				filesAdded = true;
