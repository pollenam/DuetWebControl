      ...
      <span v-else>
        ...
        {{ $t('generic.status.idle') }}
	        ...
	        <v-spacer></v-spacer>
				...
				<v-btn v-if="isPaused" class="ms-0 ms-md-2" @click="cancel" elevation="0" :disabled="uiFrozen">
...
export default {
	...
	methods: {
		...
		async stopInfiniteThenResume() {
			try {
				await this.sendCode('M991'); // stop infinite extrusion
				await this.sendCode('M24');  // triggers resume.g file
			} catch (e) {
